...
public class Snake {
    ...
    public Snake(int unitSize) {
        this(unitSize, Color.GREEN, new Color(45, 180, 0), new Point(0, 0), 'R');
    }

    public Snake(int unitSize, Color headColor, Color bodyColor) {
        this(unitSize, headColor, bodyColor, new Point(0, 0), 'R');
    }

    public Snake(int unitSize, Color headColor, Color bodyColor, Point headPos) {
        this(unitSize, headColor, bodyColor, headPos, 'R');
    }

    public Snake(int unitSize, Color headColor, Color bodyColor, Point headPos, char direction) {
        this.unitSize = unitSize;
        this.color = headColor;
        this.bodyColor = bodyColor;
        this.direction = direction;
        body = new ArrayList<>();
        body.add(new Point(headPos)); // Initial head position
        for (int i = 1; i < 6; i++) {
            int x = headPos.x, y = headPos.y;
            switch (direction) {
                case 'U': y += i * unitSize; break;
                case 'D': y -= i * unitSize; break;
                case 'L': x += i * unitSize; break;
                case 'R': x -= i * unitSize; break;
            }
    ...
    public void draw(Graphics g) {
        for (int i = 0; i < body.size(); i++) {
            if (i == 0) {
                g.setColor(color); // Head color
            } else {
                g.setColor(bodyColor); // Body color
            }
